...
export const WalletSchema = z.object({
    ...
    pinOnMain: z.boolean(),
    pinned: z.boolean(),
    isFastAccessByCurrentUser: z.boolean().default(false),
    visible: z.boolean(),
    deleted: z.boolean(),
...
const WalletDetailsCreateSchema = z
    .object({
        ...
        accountId: optionalTrimmedString(255, 'ID аккаунта не должен превышать 255 символов'),
...
export const CreateWalletSchema = z
    .object({
        ...
        walletTypeId: z.string().uuid('Укажите корректный UUID').optional().or(z.literal('')),
        currencyId: z
            .string({ message: 'Валюта обязательна' })
            .min(1, 'Валюта обязательна')
            .uuid('Укажите корректный UUID'),
        secondUserId: z.string().uuid('Укажите корректный UUID').optional().or(z.literal('')),
